...
const String ACTIVE_DAYS = 'active_days';
...
Future<Database> initDb() async {
  ...
  Database db = await sql.openDatabase(
    ...
    onUpgrade: (db, oldVersion, newVersion) async {
      await db.execute(
        'CREATE TABLE IF NOT EXISTS $VISISTED_LOCATION_TABLE (id INTEGER PRIMARY KEY, day TEXT NOT NULL, active INTEGER',
      );
...
Future<void> addDayRecords(Database db) async {
  await db.insert('ACTIVE_DAYS', {'id': 1, 'name': 'Monday', 'active': 0});
  await db.insert('ACTIVE_DAYS', {'id': 2, 'name': 'Tuesday', 'active': 0});
  await db.insert('ACTIVE_DAYS', {'id': 3, 'name': 'Wednesday', 'active': 0});
  await db.insert('ACTIVE_DAYS', {'id': 4, 'name': 'Thursday', 'active': 0});
  await db.insert('ACTIVE_DAYS', {'id': 5, 'name': 'Friday', 'active': 0});
  await db.insert('ACTIVE_DAYS', {'id': 6, 'name': 'Saturday', 'active': 0});
  await db.insert('ACTIVE_DAYS', {'id': 7, 'name': 'Sunday', 'active': 0});
}
